...
def gerar_pdf_relatorio(dados):
    ...
    story.append(Paragraph("Relatório Monitoramento Automatizado de Sistemas e Tribunais", title_style))
    ...
    if not dados:
        ...
        for row in dados:
            # 8 colunas: data, fonte, titulo, status, motivo, termo_base, palavra_encontrada, link
            data, fonte, titulo, status, motivo, termo, palavra, link = row

            texto = (
                f"<b>Data:</b> {data} <br/>"
                f"<b>Fonte:</b> {fonte} <br/>"
                f"<b>Status:</b> {status} &nbsp;|&nbsp; <b>Motivo:</b> {motivo} <br/>"
